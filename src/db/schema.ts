...
export const venues = pgTable(
  "venues",
  {
    ...
    courtCount: smallint("court_count"),
  ...
  (table) => [
    ...
    check("venues_court_count_positive", sql`${table.courtCount} is null or ${table.courtCount} > 0`),
    check("venues_rating_range", sql`${table.rating} between 0 and 5`),
    check("venues_availability_allowed", sql`${table.availability} in ('unknown', 'available', 'waitlist')`),
...
export const venueSuggestions = pgTable(
  "venue_suggestions",
  {
    id: uuid("id").defaultRandom().primaryKey(),
  ...
  (table) => [
    index("venue_suggestions_venue_idx").on(table.venueId),
    index("venue_suggestions_status_created_idx").on(table.status, table.createdAt, table.id),
    check("venue_suggestions_name_length", sql`char_length(${table.name}) between 2 and 160`),
    check("venue_suggestions_province_length", sql`char_length(${table.province}) between 1 and 80`),
    check("venue_suggestions_status_allowed", sql`${table.status} in ('pending', 'approved', 'rejected')`),
  ],
);
...
export const marketplaceListings = pgTable(
  ...
  (table) => [
    ...
    index("marketplace_listings_popularity_idx").on(table.status, table.viewCount, table.createdAt, table.id),
    check("marketplace_listings_price_nonnegative", sql`${table.price} >= 0`),
    check("marketplace_listings_view_count_nonnegative", sql`${table.viewCount} >= 0`),
