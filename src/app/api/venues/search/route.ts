...
async function search(request: Request, coordinates: GeoCoordinates | null = null) {
  ...
  try {return Response.json(await searchVenues(supabase,directoryFilters(new URL(request.url).searchParams),12,coordinates),{headers:{'Cache-Control':'private, no-store'}});}
...
export async function POST(request:Request) {
  ...
  try {
    ...
    if (!hasLatitude || !hasLongitude) return Response.json({error:'ข้อมูลตำแหน่งไม่ครบถ้วน'},{status:400});
  ...
  if (!coordinates) return Response.json({error:'ข้อมูลตำแหน่งไม่ถูกต้อง'},{status:400});
