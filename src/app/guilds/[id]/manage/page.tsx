...
export default async function GuildManagePage({
  params,
}: {
  ...
  const guildResult = await supabase
    .from("guilds")
    .select(
      "id, name, description, logo_url, cover_url, province, district, subdistrict, visibility, join_policy, level, exp_total, max_members",
    )
    .eq("id", id)
    .maybeSingle();
  ...
  if (
    ...
    !["guild_master", "officer"].includes(membershipResult.data.role)
  )
    notFound();
  ...
  const members: GuildMemberData[] = (membersResult.data ?? []).map(
    (member) => ({
  ...
  const requests: GuildJoinRequestData[] = requestRows.flatMap((request) => {
    const requestProfile = requestProfileById.get(request.user_id);
  ...
  const inventoryResult =
    itemIds.length > 0
      ? await supabase
          ...
          .eq("user_id", user.id)
          .in("item_id", itemIds)
      : { data: [] as Array<{ item_id: string; quantity: number }> };
  ...
  const expansionItems = (itemsResult.data ?? []).flatMap((item) => {
