...
export default async function MessagesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (uuidPattern.test(otherUserId) && otherUserId !== user.id) {
    const { data } = await supabase.rpc("get_or_create_direct_conversation", { p_other_user_id: otherUserId });
    const created = row(data);
    if (typeof created?.id === "string") selectedId = created.id;
  }

  const { data: memberships } = await supabase.from("direct_conversation_members").select("conversation_id").eq("user_id", user.id);
  ...
  return <MessagesBrowser conversations={conversations} selectedConversationId={selectedId || null} initialMessages={initialMessages} currentUserId={user.id} />;
}
