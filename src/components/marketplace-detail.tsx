  ...
  return (
    <main className="marketplace-detail-page">
      <div className="marketplace-detail-shell">
        <header className="groups-topbar"><Link href="/marketplace" className="groups-back"><ArrowLeft size={17} /> กลับตลาดมือสอง</Link><Link href="/" className="groups-brand"><span>Arena</span><em>-Badminton</em></Link><Link href="/friends" className="organizer-user-chip">เพื่อน</Link></header>
        ...
        <div className="marketplace-detail-columns">
          <section className="marketplace-detail-panel"><div className="marketplace-detail-panel__heading"><div><p lang="en">Item details</p><h2>รายละเอียดสินค้า</h2></div><span className={`marketplace-status marketplace-status--${listing.status}`}>{listing.status === "active" ? "ยังว่าง" : listing.status === "reserved" ? "มีคนจอง" : "ขายแล้ว"}</span></div><p className="marketplace-description">{listing.description || "ผู้ขายยังไม่ได้เพิ่มรายละเอียด"}</p><div className="marketplace-seller"><span>{listing.sellerAvatarUrl ? <img src={listing.sellerAvatarUrl} alt="" /> : <UserRound size={18} />}</span><div><strong>{listing.sellerName}</strong><small>@{listing.sellerHandle} · Level {listing.sellerLevel}</small></div><Link href={`/friends?user=${listing.sellerId}`} className="group-secondary-action"><MessageCircle size={14} /> ดูสถานะเพื่อน</Link></div></section>
          <aside>{!isSeller && listing.status === "active" ? <form className="marketplace-purchase-form" action={action}><div><p lang="en">Interested?</p><h2>ส่งคำขอซื้อ</h2><span>ระบบจะส่งคำขอให้ผู้ขาย แล้วคุยรายละเอียดผ่าน Messenger</span></div><textarea name="message" maxLength={1000} placeholder="เช่น ขอสอบถามสภาพ/นัดรับสินค้า" /><input type="hidden" name="listingId" value={listing.id} /><button type="submit" className="group-primary-action" disabled={pending}><Send size={15} /> {pending ? "กำลังส่ง..." : "ส่งคำขอซื้อ"}</button><Feedback state={state} /></form> : <section className="marketplace-purchase-locked"><PackageSearch size={25} /><strong>{isSeller ? "นี่คือประกาศของคุณ" : "รายการนี้ไม่เปิดรับคำขอแล้ว"}</strong><span>{isSeller ? "ดูแลคำขอซื้อด้านล่าง และนัดหมายกับผู้ซื้ออย่างปลอดภัย" : "สถานะสินค้านี้เปลี่ยนแปลงแล้ว"}</span></section>}</aside>
        </div>
