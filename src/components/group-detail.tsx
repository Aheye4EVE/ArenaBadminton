...
import { ArrowLeft, BarChart3, CalendarDays, Clock3, Coins, MapPin, ShieldAlert, ShieldCheck, Swords, Users } from "lucide-react";
...
const dateFormatter = new Intl.DateTimeFormat("th-TH", { dateStyle: "full", timeZone: "Asia/Bangkok" });
  ...
  return (
    <main className="groups-page group-detail-page">
      <div className="groups-shell">
        <header className="groups-topbar"><Link href="/groups" className="groups-back"><ArrowLeft size={17} /> ก๊วนทั้งหมด</Link><Link href="/" className="groups-brand" aria-label="กลับหน้าหลัก Arena-Badminton"><span>Arena</span><em>-Badminton</em></Link><Link href="/profile" className="organizer-user-chip">Profile</Link></header>
        ...
        <footer className="group-detail-footer"><span>ระบบจัดก๊วนโดย Community</span><Link href={`/moderation/report?targetType=group&targetId=${group.id}&returnTo=/groups/${group.id}`} className="venue-review-report"><ShieldAlert size={13} /> รายงานก๊วน</Link></footer>
      </div>
    </main>
  );
}
