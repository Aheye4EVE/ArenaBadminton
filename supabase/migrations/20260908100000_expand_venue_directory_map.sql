-- Let the client switch to a real directory map without making twelve
-- paginated RPC calls. The bounded 600-row limit leaves room for the current
-- directory and future additions while retaining a safe upper bound for the
-- authenticated RPC.
...
as $$
  ...
  page as (
    ...
    from ranked
    order by position
    limit greatest(1, least(coalesce(p_size, 24), 600))
    offset (
      greatest(1, least(coalesce(p_page, 1), 100000)) - 1
    ) * greatest(1, least(coalesce(p_size, 24), 600))
  )
